"""
Train Cost Risk Model using XGBoost.

Predicts likelihood of cost overruns based on project features.
"""
import sys
import argparse
import json
...
class CostRiskModelTrainer:
    """Train and evaluate cost risk prediction model."""
    
    LABEL_COL = 'cost_risk_label'
    ...
    def train(self, X_train, y_train, X_val, y_val):
        """Train XGBoost model."""
        logger.info("Training XGBoost cost risk model...")
...
def main():
    parser = argparse.ArgumentParser(description="Train cost risk model")
    parser.add_argument(
        '--data',
        type=Path,
        required=True,
    ...
    parser.add_argument(
        '--output',
        type=Path,
        default=Path('../models/cost_risk_model.pkl'),
        help='Output model path'
    )
    parser.add_argument(
        '--metadata',
        type=Path,
        default=Path('../models/cost_risk_metadata.json'),
    ...
    try:
        trainer = CostRiskModelTrainer(random_seed=args.seed)
        ...
        print(f"\n✅ Cost Risk Model trained successfully!")
        print(f"\nTest Metrics:")
